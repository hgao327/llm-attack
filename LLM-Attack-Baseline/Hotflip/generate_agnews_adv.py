...
def get_label_text(label_val):
    """Convert numeric label to English text (AG News 4-class)"""
    labels = {0: 'World', 1: 'Sports', 2: 'Business', 3: 'Sci/Tech'}
    return labels.get(int(label_val), 'Unknown')
...
if __name__=="__main__":
    print("loading data...")
    n_option = 500
    sim_threshold=0.5 # 提高阈值以生成更多样本 (原值 0.4)
    sim_string=str(sim_threshold)
    THEME="agnews" # for dataset
    ...
    # AG News 数据已在 agnews.p 中，不需要加载额外数据
    print(f"Using data from agnews.p only (no extra data loading)")
    # ========== 加载额外数据完成 ==========
    ...
    for i in range(len(classifier.params)):
        classifier.params[i].set_value(tmp[i])
